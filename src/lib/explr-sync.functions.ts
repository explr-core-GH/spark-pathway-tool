...
// Generate one-click magic-link logins for every parent_email in an
// ExplrMore camp's roster. Returns the links — admin distributes them (no
// SMTP required). Supabase creates the auth.users entry on first generate;
// subsequent calls return a fresh single-use link without duplicating users.
export const generateExplrCampLogins = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const r of (regs ?? []) as Array<{
      ...
      if (!r.parent_email) {
        results.push({
          ...
          parentEmail: null,
          link: null,
          error: "No parent_email on roster row",
        });
        continue;
      }
      try {
        ...
        const link =
          (linkData?.properties?.action_link as string | undefined) ?? null;
        results.push({
          registrationId: r.id,
          childName: r.child_name,
          parentName: r.parent_name,
          ...
          link,
          error: link ? null : "Supabase returned no action_link",
        });
      } catch (e) {
        results.push({
          registrationId: r.id,
    ...
    return { ok: true, results, totalRoster: results.length };
  });
