...
function EducatorLayout() {
  ...
  const approved = !!educator && educator.approved && (!!educator.program_type || educator.role === "admin");
  return (
    <div className="min-h-screen">
      <header className="border-b border-charcoal-100">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
          <Link to="/" className="text-base font-medium">EXPLR <span style={{ color: "var(--explr)" }}>Pathways</span> <span className="text-charcoal-400">/ {educator?.role === "admin" ? "Admin" : "Educators"}</span></Link>
